import require$$1$2, { app, dialog, BrowserWindow, ipcMain, screen, session, shell } from "electron";
...
import path$i from "path";
import require$$1$3, { exec, fork, execSync } from "child_process";
...
const debug$6 = (0, debug_1$1.default)("electron-builder");
...
class HttpExecutor {
  ...
  doApiRequest(options, cancellationToken, requestProcessor, redirectCount = 0) {
    if (debug$6.enabled) {
      debug$6(`Request: ${safeStringifyJson(options)}`);
    }
  ...
  handleResponse(response, options, cancellationToken, resolve, reject, redirectCount, requestProcessor) {
    var _a;
    if (debug$6.enabled) {
      debug$6(`Response: ${response.statusCode} ${response.statusMessage}, request options: ${safeStringifyJson(options)}`);
    }
...
function safeStringifyJson(data, skippedNames) {
  return JSON.stringify(data, (name3, value) => {
    if (name3.endsWith("Authorization") || name3.endsWith("authorization") || name3.endsWith("Password") || name3.endsWith("PASSWORD") || name3.endsWith("Token") || name3.includes("password") || name3.includes("token") || skippedNames != null && skippedNames.has(name3)) {
...
class UUID {
  ...
  static v5(name3, namespace) {
    return uuidNamed(name3, "sha1", 80, namespace);
  }
  toString() {
    if (this.ascii == null) {
...
function uuidNamed(name3, hashMethod, version2, namespace, encoding = UuidEncoding.ASCII) {
  const hash = (0, crypto_1$1.createHash)(hashMethod);
  const nameIsNotAString = typeof name3 !== "string";
  if (nameIsNotAString && !Buffer.isBuffer(name3)) {
...
class XElement {
  constructor(name3) {
    ...
    if (!isValidName(name3)) {
      throw (0, error_1.newError)(`Invalid element name: ${name3}`, "ERR_XML_ELEMENT_INVALID_NAME");
    }
  }
  attribute(name3) {
    const result = this.attributes === null ? null : this.attributes[name3];
    if (result == null) {
      throw (0, error_1.newError)(`No attribute "${name3}"`, "ERR_XML_MISSED_ATTRIBUTE");
    }
    return result;
  }
  ...
  element(name3, ignoreCase = false, errorIfMissed = null) {
    const result = this.elementOrNull(name3, ignoreCase);
    if (result === null) {
      throw (0, error_1.newError)(errorIfMissed || `No element "${name3}"`, "ERR_XML_MISSED_ELEMENT");
    }
    return result;
  }
  elementOrNull(name3, ignoreCase = false) {
    if (this.elements === null) {
      return null;
    }
    for (const element of this.elements) {
      if (isNameEquals(element, name3, ignoreCase)) {
        return element;
      }
    }
    return null;
  }
  getElements(name3, ignoreCase = false) {
    if (this.elements === null) {
      return [];
    }
    return this.elements.filter((it) => isNameEquals(it, name3, ignoreCase));
  }
  elementValueOrEmpty(name3, ignoreCase = false) {
    const element = this.elementOrNull(name3, ignoreCase);
...
function isValidName(name3) {
  return NAME_REG_EXP.test(name3);
}
function isNameEquals(element, name3, ignoreCase) {
  const elementName = element.name;
  return elementName === name3 || ignoreCase === true && elementName.length === name3.length && elementName.toLowerCase() === name3.toLowerCase();
}
...
function makeSnippet$1(mark, options) {
  ...
  for (i = 1; i <= options.linesBefore; i++) {
    ...
    result = common$5.repeat(" ", options.indent) + padStart((mark.line - i + 1).toString(), lineNoLength) + " | " + line.str + "\n" + result;
  }
  line = getLine(mark.buffer, lineStarts[foundLineNo], lineEnds[foundLineNo], mark.position, maxLineLength);
  result += common$5.repeat(" ", options.indent) + padStart((mark.line + 1).toString(), lineNoLength) + " | " + line.str + "\n";
...
function Type$e(tag, options) {
  ...
  Object.keys(options).forEach(function(name3) {
    if (TYPE_CONSTRUCTOR_OPTIONS.indexOf(name3) === -1) {
      throw new YAMLException$3('Unknown option "' + name3 + '" is met in definition of "' + tag + '" YAML type.');
    }
  });
...
var timestamp$2 = new Type$5("tag:yaml.org,2002:timestamp", {
  kind: "scalar",
...
async function createTempUpdateFile(name3, cacheDir, log) {
  let nameCounter = 0;
  let result = path$4.join(cacheDir, name3);
  for (let i = 0; i < 3; i++) {
    try {
      ...
      result = path$4.join(cacheDir, `${nameCounter++}-${name3}`);
    }
  }
  return result;
}
...
util$3.blockmapFiles = blockmapFiles;
...
class PrivateGitHubProvider extends GitHubProvider_1$1.BaseGitHubProvider {
  ...
  resolveFiles(updateInfo) {
    return (0, Provider_1.getFileList)(updateInfo).map((it) => {
      const name3 = path$1.posix.basename(it.url).replace(/ /g, "-");
      const asset = updateInfo.assets.find((it2) => it2 != null && it2.name === name3);
      if (asset == null) {
        throw (0, builder_util_runtime_1$5.newError)(`Cannot find asset "${name3}" in: ${JSON.stringify(updateInfo.assets, null, 2)}`, "ERR_UPDATER_ASSET_NOT_FOUND");
      }
      return {
...
function computeOperations(oldBlockMap, newBlockMap, logger2) {
  ...
  const operations = [];
  const name3 = blockMapFile.name;
  const oldEntry = nameToOldBlocks.get(name3);
  if (oldEntry == null) {
    throw new Error(`no file ${name3} in old blockmap`);
  }
  const newFile = nameToNewBlocks.get(name3);
  let changedBlockCount = 0;
  const { checksumToOffset: checksumToOldOffset, checksumToOldSize } = buildChecksumMap(nameToOldBlocks.get(name3), oldEntry.offset, logger2);
...
function requireAppUpdater() {
  ...
  const builder_util_runtime_12 = out;
  const crypto_12 = require$$0$5;
  const os_12 = require$$0$2;
...
function requireBaseUpdater() {
  ...
  const child_process_12 = require$$1$3;
  ...
  let BaseUpdater$1 = class BaseUpdater extends AppUpdater_1.AppUpdater {
    ...
    wrapSudo() {
      const { name: name3 } = this.app;
      const installComment = `"${name3} would like to update"`;
    ...
    (0, child_process_1.execFile)(`set "PSModulePath=" & chcp 65001 >NUL & powershell.exe`, ["-NoProfile", "-NonInteractive", "-InputFormat", "None", "-Command", `"Get-AuthenticodeSignature -LiteralPath '${tempUpdateFile}' | ConvertTo-Json -Compress"`], {
      ...
      try {
        ...
        if (data.Status === 0) {
          ...
          let match = false;
          for (const name3 of publisherNames) {
            const dn = (0, builder_util_runtime_1.parseDn)(name3);
            if (dn.size) {
              const allKeys = Array.from(dn.keys());
              match = allKeys.every((key) => {
                return dn.get(key) === subject.get(key);
              });
            } else if (name3 === subject.get("CN")) {
              logger2.warn(`Signature validated using only CN ${name3}. Please add your full Distinguished Name (DN) to publisherNames configuration`);
              match = true;
            }
            if (match) {
              resolve(null);
        ...
        const result = `publisherNames: ${publisherNames.join(" | ")}, raw info: ` + JSON.stringify(data, (name3, value) => name3 === "RawData" ? void 0 : value, 2);
    ...
    doInstall(options) {
      ...
      this.spawnLog(options.installerPath, args).catch((e) => {
        ...
        if (errorCode === "UNKNOWN" || errorCode === "EACCES") {
          ...
          require$$1$2.shell.openPath(options.installerPath).catch((err) => this.dispatchError(err));
        } else {
          this.dispatchError(e);
        }
      });
...
var ERR_INVALID_OPT_VALUE = errors$1.codes.ERR_INVALID_OPT_VALUE;
...
function requireFrom() {
  ...
  var ERR_INVALID_ARG_TYPE = errors$1.codes.ERR_INVALID_ARG_TYPE;
...
function require_stream_readable() {
  ...
  var _require$codes = errors$1.codes, ERR_INVALID_ARG_TYPE = _require$codes.ERR_INVALID_ARG_TYPE, ERR_STREAM_PUSH_AFTER_EOF = _require$codes.ERR_STREAM_PUSH_AFTER_EOF, ERR_METHOD_NOT_IMPLEMENTED = _require$codes.ERR_METHOD_NOT_IMPLEMENTED, ERR_STREAM_UNSHIFT_AFTER_END_EVENT = _require$codes.ERR_STREAM_UNSHIFT_AFTER_END_EVENT;
  var StringDecoder;
...
function requireConsole$1() {
  ...
  console_1$1 = class Console extends TransportStream3 {
     ...
     * @private
     */
    _stringArrayToSet(strArray, errMsg2) {
      if (!strArray) return {};
      errMsg2 = errMsg2 || "Cannot make set from type other than Array of string elements";
...
function require_stream_transform() {
  ...
  var _require$codes = errors$1.codes, ERR_METHOD_NOT_IMPLEMENTED = _require$codes.ERR_METHOD_NOT_IMPLEMENTED, ERR_MULTIPLE_CALLBACK = _require$codes.ERR_MULTIPLE_CALLBACK, ERR_TRANSFORM_ALREADY_TRANSFORMING = _require$codes.ERR_TRANSFORM_ALREADY_TRANSFORMING, ERR_TRANSFORM_WITH_LENGTH_0 = _require$codes.ERR_TRANSFORM_WITH_LENGTH_0;
...
function requirePipeline() {
  ...
  var _require$codes = errors$1.codes, ERR_MISSING_ARGS = _require$codes.ERR_MISSING_ARGS, ERR_STREAM_DESTROYED = _require$codes.ERR_STREAM_DESTROYED;
  function noop2(err) {
    if (err) throw err;
  }
  function isRequest(stream2) {
...
function requireTailFile() {
  ...
  const { Stream: Stream2 } = readableExports;
  function noop2() {
  }
...
function requireFile() {
  ...
  const zlib = require$$15;
  const { MESSAGE: MESSAGE2 } = tripleBeam;
  const { Stream: Stream2, PassThrough } = readableExports;
  const TransportStream3 = winstonTransportExports;
...
function requireLogger() {
  ...
  hasRequiredLogger = 1;
  const { Stream: Stream2, Transform } = readableExports;
...
var createLogger$1 = function(opts = {}) {
...
(function(exports) {
  ...
  exports.Transport = winstonTransportExports;
...
const __dirname$1 = path$i.dirname(__filename$1);
const packageJsonPath$1 = path$i.join(__dirname$1, "../package.json");
const packageJson$1 = JSON.parse(fs$j.readFileSync(packageJsonPath$1, "utf-8"));
...
const logDir = path$i.dirname(logFilePath);
if (!fs$j.existsSync(logDir)) {
  fs$j.mkdirSync(logDir, { recursive: true });
}
fs$j.writeFileSync(logFilePath, "");
...
const __dirname = path$i.dirname(__filename);
const packageJsonPath = path$i.join(__dirname, "../package.json");
const packageJson = JSON.parse(fs$j.readFileSync(packageJsonPath, "utf-8"));
const logger = setupLogger("main.js");
setupUncaughtExceptionHandler();
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("titan", process.execPath, [path$i.resolve(process.argv[1])]);
  }
} else {
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      preload: fs$j.existsSync(path$i.join(__dirname, "preload.mjs")) ? path$i.join(__dirname, "preload.mjs") : path$i.join(__dirname, "preload.js"),
      sandbox: true
    }
  });
...
function createSplashWindow() {
  ...
  splashWindow.loadFile(path$i.join(__dirname, "../splash.html"));
  const primaryDisplay = screen.getPrimaryDisplay();
  const { width, height } = primaryDisplay.workAreaSize;
...
function startServer() {
  serverProcess = fork(path$i.join(__dirname, "../server/server.js"), [], {
...
if (!gotTheLock) {
  ...
  app.on("second-instance", (event, commandLine, workingDirectory) => {
    ...
    dialog.showMessageBox(mainWindow, {
      ...
      icon: path$i.join(__dirname, "../icons/Titan.ico")
    });
  });
  app.on("ready", () => {
    createSplashWindow();
...
function gracefulShutdown() {
  ...
  if (!updateDownloaded) {
    ...
    shutdownDialog.loadFile(path$i.join(__dirname, "../shutdown.html"));
  }
    ...
    new Promise((resolve, reject) => {
      if (serverProcess && !serverProcess.killed && !serverProcess.exited && serverProcess.connected) {
        ...
        const onError = (error2) => {
          logger.error("Server process error during shutdown:", error2);
          cleanup();
          reject(error2);
        };
        const cleanup = () => {
...
ipcMain.handle("fetch-custom-scripts", async () => {
  ...
  const scripts2 = [];
  try {
    const files = fs$j.readdirSync(configFolderPath);
    files.forEach((file2) => {
      const ext = path$i.extname(file2);
      if ([".bat", ".ps1"].includes(ext.toLowerCase())) {
        const fullPath = path$i.join(configFolderPath, file2);
        scripts2.push({
          fileName: path$i.parse(file2).name,
          path: fullPath,
  ...
  return { success: true, scripts: scripts2 };
});
